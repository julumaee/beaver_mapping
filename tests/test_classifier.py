...
def _make_chips_and_manifest(tmp_path: Path, n=10) -> str:
    ...
    for i in range(n):
        # Positive: high NIR, low green → vegetation/beaver signature
        chip = np.zeros((3, 512, 512), dtype=np.uint8)
        chip[0] = rng.integers(150, 220, (512, 512))  # NIR high
        chip[1] = rng.integers(40, 80, (512, 512))    # Red low
        chip[2] = rng.integers(40, 80, (512, 512))    # Green low
        p = tmp_path / f"pos_{i}.npy"
        np.save(str(p), chip)
        rows.append({"path": str(p), "label": 1, "x": 0, "y": 0})

    for i in range(n):
        # Negative: balanced bands → generic non-beaver area
        chip = rng.integers(60, 100, (3, 512, 512), dtype=np.uint8)
        p = tmp_path / f"neg_{i}.npy"
        np.save(str(p), chip)
        rows.append({"path": str(p), "label": 0, "x": 0, "y": 0})
